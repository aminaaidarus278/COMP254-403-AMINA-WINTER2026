...
public class CircularlyLinkedList<E> implements Cloneable{
  ...
  public CircularlyLinkedList<E> clone() throws CloneNotSupportedException {
    ...
    // 1) Identify the head of original
    Node<E> originalHead = this.tail.getNext();

    // 2) Create the first node of the copy (copy head)
    Node<E> newHead = new Node<>(originalHead.getElement(), null);

    // 3) Walk through the original circle exactly size-1 more times
    ...
    // 4) Close the circle: newTail.next = newHead
    newTail.setNext(newHead);

    // 5) Set other's tail and size
    other.tail = newTail;
  ...
  public static void main(String[] args) {
    ...
    // add elements to original list
    list1.addLast("A");
    list1.addLast("B");
    list1.addLast("C");

    System.out.println("\n--------exercise 3- Circularly Linked Lists ");
    System.out.println("Original list: " + list1);

    try {
      // clone the list
      CircularlyLinkedList<String> list2 = list1.clone();
